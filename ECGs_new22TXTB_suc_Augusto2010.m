...
%% 21 Jan 2010,  13 martiou 2010 Yulear method
...
for q=1:a7
    ...
    clear current_name
       ...
       % saveas(gcf, 'CorrectedRR', 'fig')
        ...
        set(hopts, 'Fs', Fs_buffer, 'NFFT', NFFT)
        	 
        hpsd=psd(h,buffer, hopts);
        figure; plot(hpsd); saveas(gcf, 'spectrumYulear','fig')
        power=avgpower(hpsd);
...
%         saveas(gcf, 'Spectrum VLF 0p003 to 0p04', 'fig');
...
%         set(gca, 'YLim', ylim);grid on; zoom on;title(['RR Frequency Spectrum' name ]); xlabel('Frequency (Hz)');
%         saveas(gcf, 'Spectrum LF 0p04 to 0p15', 'fig');
%         
%         
...
%         set(gca, 'YLim', ylim);grid on; zoom on;title(['RR Frequency Spectrum' name ]); xlabel('Frequency (Hz)');
%        %saveas(gcf, 'Spectrum HF 0p04 to 0p15', 'fig');

        %% total power HF LF 0.003 - 0.4 Hz
...
%         figure; plot(F(freqVLF1:freqHF),P(freqVLF1:freqHF,1));
%         set(gca, 'YLim', ylim);grid on; zoom on;title(['RR Frequency Spectrum' name ]); xlabel('Frequency (Hz)');
%         %saveas(gcf, 'Spectrum TOTAL 0 to 0p15', 'fig')

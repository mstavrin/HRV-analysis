...
%% 20 Jan 2010 Welch's method     -->>> h=spectrum.welch('hamming');
%%% the ex ECGs_new16TXTB_suc.m
%%% Now its automated!!!!!!!!
%% August 2010, 21, with uipickfiles instead of uigetfiles
%% 22 Sept 2010
clear all
...
%     %% reverse the signal if the max is in the negative frequencies
%     [n x]=hist(ecg_sinal1);
%     indexmaxfr=find(n>mean(n));
%     if x(indexmaxfr)<0
%        display('negative')
%        %ecg_sinal=-ecg_sinal;
%     end
%     
%    sumECG=sum(ecg_sinal);
%    if sumECG<0
%          ecg_sinal=-ecg_sinal;
%    end
%      figure; plot(ecg_sinal); title(['final ECG ' name]);  saveas(gcf, 'ECGsinal_final', 'fig')
    clear current_name
        ...
        for i=2:(length(intervaloRR)-1) % xekiname apo to 2 gia g;itwsoume to x(-1)
	 ...
	 saveas(gcf, 'RR_interpolated', 'fig')
		...
		tbefore=1:length(intervaloRR); %% eikonikos pinakas xronou gia na eimaste sumbatoi me to trial4_4.m
        ...
        Freqresnew=Fsnew/NN % this is the current frequency resolution
       % Fsnew=1/0.0005; %Fs= F(2)*length(interpol_intervaloRR) % H nea sampling frequency basei tis interpolation  2000 Hz
        save interpol_intervaloRR interpol_intervaloRR
        %save interpol2 interpol2 
        save Fsnew Fsnew
      
        %define if we work on the interpolated or on the sample signal
        buffer=(interpol_intervaloRR-meanINTRR)/std(interpol_intervaloRR); 
        Fs_buffer=Fsnew;
		figure; plot(intervaloRR); hold on; plot(buffer, 'r'); title('interpolated and normalized interpolated in red')
        
        % Power spectrum estimation of the interploated RR, by Welch
        NN=length(interpol_intervaloRR);						
        p2=nextpow2(NN); % calculate length of window
        NFFT=2^p2;
        xnew=[buffer zeros(1,NFFT-length(buffer))];
		buffer=xnew; 
         figure; plot(xnew); title('zero padded signal');
	%
		% Power spectrum estimation of the interploated RR, by Welch
	  ...
	  meanINTRR=mean(interpol_intervaloRR);
        ...
        set(hopts, 'Fs', Fs_buffer, 'SpectrumType', 'onesided', 'centerdc', false)
...
%          msspectrum(h,buffer,hopts)
        ...
        %hpsd=psd(h,buffer, hopts);
        figure; plot(hpsd); 
        saveas(gcf, 'psdwelch_interpol', 'fig')
        %
        power=avgpower(hpsd)/(Fsnew)
        Parsevalaki=(sum(buffer.^2))/(length(buffer)*Fsnew) %   
        
        
%         NFFT=2^nextpow2(NN);
%         buffer=interpol_intervaloRR;%(interv(2*j-1):interv(2*j),i); dati(1:131073,i);
%         
%        
...
%          hopts=psdopts(h,buffer);
%          set(hopts, 'Fs', Fsnew, 'SpectrumType', 'onesided', 'centerdc', true, 'NormalizedFrequency', false);
%          %hopts.FrequencyVector = F;
%          msspectrum(h,x,hopts)
%          hpsd=psd(h,buffer, hopts);%% kanei plot
%          
%          power=avgpower(hpsd)/(Fsnew); figure(23); plot(hpsd); title([' Total Power ' num2str(power)]) 
%         
...
%         
        ...
        % From Parseval theorem the sum of squares of the signal itself divided by the lenght of the signal gives
            % the power spectral density and its the same from avgpower (if
            % it is twosided. if avgpower is calculated on onesided then
            % its half the one given by the Parseval formula




% 
%         %find the 1 Hz and stop plotting there
%         find1hz=find(F<1);
%         freq1hz=find1hz(end);
%         %Plot
%         P=P';
%         figure; plot(F(1:freq1hz),P(1:freq1hz,1));%title([' EEG_channel ' s{i}]) %%% (1:469) (1:469)
%         set(gca, 'YLim', ylim);grid on; zoom on;title(['RR Frequency Spectrum' name ]); xlabel('Frequency (Hz)');
%         saveas(gcf, 'Spectrum until 1Hz', 'fig'); close(gcf);
% 
%         %plot all  %find the 40 Hz and stop plotting there
%         find40hz=find(F<40);
%         freq40hz=find40hz(end);
%         figure; plot(F(1:freq40hz),P(1:freq40hz,1));%title([' EEG_channel ' s{i}]) %%% (1:469) (1:469)
%         set(gca, 'YLim', ylim);grid on; zoom on;title(['RR Frequency Spectrum' name ]); xlabel('Frequency (Hz)');
%         saveas(gcf, 'Spectrum until 40Hz', 'fig'); close(gcf);
...
%         figure; plot(F(freqVLF1:freqVLF2),P(freqVLF1:freqVLF2,1));%title([' EEG_channel ' s{i}]) %%% (1:469) (1:469)
%         set(gca, 'YLim', ylim);grid on; zoom on;title(['RR Frequency Spectrum' name ]); xlabel('Frequency (Hz)');
...
%         figure; plot(F(freqVLF2:freqLF),P(freqVLF2:freqLF));%title([' EEG_channel ' s{i}]) %%% (1:469) (1:469)
%         set(gca, 'YLim', ylim);grid on; zoom on;title(['RR Frequency Spectrum' name ]); xlabel('Frequency (Hz)');
...
%         figure; plot(F(freqVLF1:freqHF),P(freqVLF1:freqHF,1));%title([' EEG_channel ' s{i}]) %%% (1:469) (1:469)
%         set(gca, 'YLim', ylim);grid on; zoom on;title(['RR Frequency Spectrum' name ]); xlabel('Frequency (Hz)');
